...
运行中的阶段、可测量进度和闭集错误码通过签名 `control.v2` 状态消息传递。Worker
只更新一个可覆盖的加密状态评论，避免在 Mailbox 中累积明文或无限评论；检查点和
最终结果仍使用独立加密 Artifact。客户端会校验签名、task ID、input hash 和递增
sequence，无法验证时不会显示进度或成功状态。
